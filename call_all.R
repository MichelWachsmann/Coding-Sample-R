...
# Professor: Gemma Dipoppa (Columbia) and Annalisa Pezone (NYU)
# Author: Michel Wachsmann (FGV EESP)
# Last Modification: 2026-03-24
# Description: This script runs all steps of the Research Assistant Assessment
#              pipeline. It sources the data cleaning, descriptive, and results 
#              scripts, and then renders the final PDF report. The report is 
#              first compiled inside reports/intermediate so that all LaTeX logs
#              and auxiliary files remain there. The final PDF is then copied to 
#              the project root.
...
required_dirs <- c(
  ...
  # Documents (optional but consistent with README)
  here("documents")
)
